...
class TechnicalIndicators:
    ...
    def calculate_macd(self, fast_period: int = 12, slow_period: int = 26, signal_period: int = 9, 
        ...
        logger.info(f"计算MACD完成：快线={fast_period}, 慢线={slow_period}, 信号线={signal_period}")
        return dif, dea, macd
    
    def calculate_boll(self, period: int = 20, std_dev: float = 2.0, column: str = 'close') -> Tuple[pd.Series, pd.Series, pd.Series]:
        """
    ...
    def calculate_kdj(self, k_period: int = 9, d_period: int = 3, j_period: int = 3) -> Tuple[pd.Series, pd.Series, pd.Series]:
        """
        计算KDJ指标 (随机指标)
        ...
        logger.info(f"计算KDJ完成：K周期={k_period}, D周期={d_period}, J周期={j_period}")
        return k, d, j
    ...
    def calculate_all(self, indicators: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        if indicators is None:
            indicators = ['ma', 'ema', 'rsi', 'macd', 'boll', 'kdj', 'atr', 'obv', 'cci', 'williams_r']
        ...
        for indicator in indicators:
            try:
                if indicator == 'ma':
                    ...
                    result['MA7'] = self.calculate_ma(7)
                    result['MA10'] = self.calculate_ma(10)
                    result['MA15'] = self.calculate_ma(15)
                    result['MA20'] = self.calculate_ma(20)
                    result['MA30'] = self.calculate_ma(30)
                    result['MA60'] = self.calculate_ma(60)
                    result['MA80'] = self.calculate_ma(80)
                elif indicator == 'ema':
                    result['EMA12'] = self.calculate_ema(12)
                    result['EMA26'] = self.calculate_ema(26)
                elif indicator == 'rsi':
                    result['RSI6'] = self.calculate_rsi(6)
                    result['RSI12'] = self.calculate_rsi(12)
                    result['RSI24'] = self.calculate_rsi(24)
                elif indicator == 'macd':
